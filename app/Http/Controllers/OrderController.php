...
class OrderController extends Controller
{
    public function process()
	{
		...
		$cities = isset(auth()->user()->city_id) ? $this->getCities(auth()->user()->province_id) : [];
 		...
 		$productOwnerIds = $items->map(function ($item) {
			return $item->associatedModel->user_id;
		})->unique();

 		$bankAccounts = \App\Models\BankAccount::whereIn('user_id', $productOwnerIds)->get();

		return view('frontend.orders.checkout', compact('items', 'totalWeight', 'provinces', 'cities', 'bankAccounts'));
	}
	...
	public function checkout(Request $request)
	{
		...
		$order = \DB::transaction(function () use ($params) {
			...
			$orderParams = [
				...
				'payment_status' => Order::CREATED, // Set payment status to PENDING
		...
		if ($request->hasFile('payment_proof')) {
            ...
            $order->bank_account_id = $request->bank_account_id;
            $order->save();
        }

		return redirect()->route('checkout.received', $order->id)->with([
			'message' => 'Order placed successfully! Please make the payment and wait for admin verification.',
			'alert-type' => 'success',
		]);
	}
